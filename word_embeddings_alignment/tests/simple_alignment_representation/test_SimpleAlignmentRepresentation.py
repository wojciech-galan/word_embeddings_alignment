...
from word_embeddings_alignment.src.simple_alignment_representation import AttributeNotSet


@pytest.fixture(scope="module")
def simple_alignment() -> SimpleAlignmentRepresentation:
	s = SimpleAlignmentRepresentation(9, 4, 3)
	s._seq1 = ['C', 'T', 'A']
	s._seq2 = ['C', '-', 'A']
	return s
...
def test_internal_data(simple_alignment: SimpleAlignmentRepresentation):
	...
	assert simple_alignment._seq1_end_position == 4
...
def test_properties(simple_alignment: SimpleAlignmentRepresentation):
	...
	assert simple_alignment.seq1_end_position == 4
	assert simple_alignment.seq2_end_position == 3
	with pytest.raises(AttributeNotSet):
		simple_alignment.seq1_start_position
	with pytest.raises(AttributeNotSet):
		simple_alignment.seq2_start_position
	simple_alignment.set_start_position(1, 0)
	assert simple_alignment.seq1_start_position == 1
	assert simple_alignment.seq2_start_position == 0
